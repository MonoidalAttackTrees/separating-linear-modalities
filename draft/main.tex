...
\begin{definition}
  ...
  \item a symmetric monoidal closed category
  	$(\cat{L},I,\otimes,\limp)$,
  \item a symmetric monoidal comonad \todo{Replace $(!,\q{})$ with just $!$.}$((!,\q{}),\varepsilon,\delta)$ on
...
\begin{definition}
  \label{def:weakening}
  A \todo{Remove modified for each of our new linear category
    definitions.  Also, don't use $\e{}$ make up a different name
    because we will use $e$ for the exchange
    modality.}\textbf{(modified) linear category with weakening},
  $(\cat{L},w,\e{})$, is specified by
  \begin{itemize}
  \item a monoidal closed category $(\cat{L},I,\otimes)$,
    \todo[inline]{Because we are non-symmetric closure is going
    to be different than the usual notion of ``closed'', and so for
    now, let's remove ``closed'' from the definitions.  For what we
    are proving at this point being closed does not play an important
    role.  We will add it back in later.}
  \item a monoidal comonad \todo{Replace $(w,\q{})$ with $w$, also,
    don't use $\q{}$ for all of these definitions, because we will need
    to keep them all straight, and this can cause confusion.}
    $((w,\q{}),\varepsilon,\delta)$ on $\cat{L}$ with
...
\begin{definition}
  ...
  \todo[inline]{remove ``modified''}
  $(\cat{L},c,\d{}^1,\d{}^2)$, is specified by
  \todo[inline]{Instead of $\d{}^i$ what do you think of $\mathsf{contraL}$ and $\mathsf{contraR}$?}
  \begin{itemize}
  \item a monoidal \todo{remove ``closed''} closed category $(\cat{L},I,\otimes)$,
  \item a monoidal comonad \todo{replace $(c,\q{})$ with just $c$.} $((c,\q{}),\varepsilon,\delta)$ on $\cat{L}$ with
  ...
  \begin{itemize}     
    \item The comonad law
      ...
      \begin{mathpar}
        ...
        \efig
      \end{mathpar}
      is satisfied by the following diagram chasing. (1) and (3) commutes by
      the naturality of $\delta^c$. (2), (4) and (5) commute by the
      conditions of $dist$. 
      \begin{mathpar}
      \bfig
        ...
        \ptriangle(1000,-200)/``/<400,400>[
          (1)``;
          ``]
        \ptriangle(1300,-600)/``/<400,400>[
          (2)``;
          ``]
        \ptriangle(500,-700)/``/<400,400>[
          (3)``;
          ``]
        \ptriangle(1850,-150)/``/<400,400>[
          (4)``;
          ``]
        \ptriangle(2100,-700)/``/<400,400>[
          (5)``;
          ``]
      \efig
      \end{mathpar}
      \todo[inline]{I am not sure how the previous diagram follows
        ...
        first diagram, and then fill it in with diagrams.}
    \item The comonad law
      ...
      components $\delta_{w^2A}^c$ and $\varepsilon_{w^2A}^c$. (3), (4), (5)
      and (6) commute by the definition of $dist$.
      \begin{mathpar}
      \bfig
      ...
      \todo[inline]{Diagram 4 is not a diagram notice there is no
        ...
        intersecting with other arrows.}  The right triangle is
      expanded in the following diagram chasing. (1) commutes by the
      naturality of $\delta^c$. (2) is the comonad law for $c$ with
      components $\delta_{wA}^c$ and $\varepsilon_{wA}^c$. (3) is the
      comonad law for $w$ with components $\delta_A^w$ and
      $\varepsilon_A^w$. (3) and (4) commute by the definition of
      $dist$.
      \begin{mathpar}
      \bfig
...
\begin{comment}
  These will go into their own lemma/proof.

\item $\e{A}:cwA\mto I$ is defined as in
    \begin{mathpar}
      \bfig
      \Atrianglepair/<-`->`->`->`->/<600,600>[
        wcA`
	...
	I;
      ...
      \efig
      \end{mathpar}
      The left triangle commutes by the definition of $dist$
      and the right triangle commutes by the definition of
      $\e{}^w$.

    \item $\d{A}:cwA\mto cwA\otimes cwA$ is defined as in
      \begin{mathpar}
        \bfig
        \square/->`->``->/<1050,600>[
          cwA`
        ...
        \efig
      \end{mathpar}
      The left part of the diagram commutes by the definitions of
