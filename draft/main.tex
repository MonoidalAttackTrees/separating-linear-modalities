...
\begin{definition}
  ...
  specified by
  \todo{I used $cw$ instead of $!$ so that it won't be confused with 
    the $!$ in linear category when we talk about the embedding.}
  \begin{itemize}
    \item a monoidal category $(\cat{L},I,\otimes)$;
...
\begin{proof}
  ...
  \item $F(A)=A$ for other basic $A$.
    \todo{Don't know how to express this.}
  \end{itemize}
  ...
  \item First, we need to show that $\hat{\cat{L}}$ is symmetric monoidal
    ...
    \end{align*}
    \todo[inline]{$\hat\lambda$ and $\hat\rho$ don't have inverses by this
      definition because we can't add an $e$ in front of $A$. I tried
      $F(A)=eA$ for basic $A$ as below, which seems to work.}
    \begin{align*}
    ...
    \todo[inline]{Or, if $F(A\otimes B)=F(A)\otimes F(B)$, $F(A)=eA$ for basic $A$,
      $F(I)=I$, we can have a much simpler solution as follows:}
    \begin{align*}
