...
By the definiiton of the distribute law $dist$ and the comonad laws of $c$
and $w$, the following two diagrams also commute:
\begin{mathpar}
\bfig
  \Vtriangle/->`->`<-/<500,400>[
    cwcA`
...
\efig
\end{mathpar}
shown by the diagram chasings below:
\begin{mathpar}
\bfig
...
% Definition: Lambek category with !
\begin{definition}
  \label{def:Lambek-!}
  A \textbf{Lambek category with $!$}, $(\cat{L},!,\w{},\c{})$, is
  specified by
  \begin{itemize}
    \item a monoidal category $(\cat{L},I,\otimes)$;
  ...
  \end{itemize}
  where $!$ is the composite monoidal comonad $cw$ defined in the proof of
  ...
  \efig
  \end{mathpar}
  for any coalgebra morphism $f:(!A,\delta_A)\mto(!B,\delta_B)$ between
  free coalgebras.
\end{definition}
...
\begin{lemma}
  ...
  Let $(\cat{L},!,\w{},\c{})$ be a Lambek category with $!$. Then the
...
\begin{proof}
  In the proof, we will write $cwA$ instead of $!$ to see the interactions
  between the comonads $c$ and $w$ more easily. 
  ...
  \begin{itemize}
    ...
    \item $\c{A}:cwA\mto cwA\otimes cwA$ is defined as below. The left part
      of the diagram commutes by the definitions of $\cL{}$ and of $\cR{}$,
      and the right part commutes because $\cat{L}$ is monoidal.
      \begin{mathpar}
        \bfig
        \square/->`->``->/<1050,600>[
          cwA`
  ...
  \begin{itemize}
    ...
    \item[1.] Each $(cwA,\w{A},\c{A})$ is a comonoid.
      ...
      conditions of the Lambek category with $!$. And (5) is the definition
      of $\c{}$.
...
\begin{definition}
  ...
  \item a monoidal comonad $(e,\varepsilon,\delta)$ on $\cat{L}$ with
    $\q{A,B}:eA\otimes eA\mto e(A\otimes B)$ and \\
    $\q{I}:I\mto eI$, and
  \item a monoidal natural isomorphism $\e{}$ on $\cat{L}$ with
    components \\
    $\e{A,B}:eA\otimes eB\mto eB\otimes eA$,
  \end{itemize}
...
\begin{definition}
  \label{def:distEx}
  Given two comonads $(!,\varepsilon^!,\delta^!)$ and
  $(e,\varepsilon^e,\delta^e)$ on a category $\cat{L}$ such that
  $(\cat{L},!,\w{},\c{})$ is a Lambek category with $!$ and
  $(\cat{L},e,\e{})$ is a Lambek category with exchange, we define a
  \textbf{distributive law for exchange} of $!$ over $e$ to be a natural
  isomorphism with components $distEx_A:!eA\mto e!A$, subject to the
  following coherence diagrams:
  \begin{mathpar}
    \bfig
      \Vtriangle/<-`<-`->/<500,400>[
...
Notice the difference between $dist$ of $c$ over $w$ and $distEx$ of $!$
over $e$. While $dist$ is a natural transformation, $distEx$ is a natural
isomorphism.


% Lemma: !e is a monoidal comonad
\begin{lemma}
  \label{lem:compose-!e}
  let $(!,\varepsilon^!,\delta^!)$ and $(e,\varepsilon^e,\delta^e)$ be two
  monoidal comonads on a Lambek category with $!$ and exchange
  $(\cat{L},I,\otimes,!,\w{},\c{},e,\e{})$. Then the composition of $!$
  and $e$ using the distributive law for exchange $distEx_A:!eA\mto e!A$
  is a monoidal comonad $(!e,\varepsilon,\delta)$ on $\cat{L}$.
\end{lemma}

\begin{proof}
  Suppose $(!,\varepsilon^!,\delta^!)$ and $(e,\varepsilon^e,\delta^e)$
  are monoidal comonads, and \\
  $(\cat{L},I,\otimes,!,\w{},\c{},e,\e{})$ is a
  Lambek category with $!$ and exchange. Since by definition
  $!,e:\cat{L}\mto\cat{L}$ are monoidal functors, we know that their
  composition \\
  $!e:\cat{L}\mto\cat{L}$ is a monoidal functor:
  \begin{align*}
    \q{A,B} &: !eA\otimes !eB\mto !e(A\otimes B)  \\
    \q{A,B} &= !\q{A,B}^e\circ\q{eA,eB}^!       \\
    \q{I}   &: I\mto !eI                          \\
    \q{I}   &= !\q{I}^e\circ\q{I}^!
  \end{align*}
...
\begin{definition}
  ...
  A \textbf{split linear category} $(\cat{L},!,e)$ is specified
  by
  \begin{itemize}
  \item a monoidal category $(\cat{L},I,\otimes)$;
  \item a monoidal comonad $(!,\varepsilon^!,\delta^!)$ with monoidal 
    natural transformations $\w{}$ and $\c{}$ s.t. $(\cat{L},!,\w{},\c{})$
    is a Lambek category with $!$;
  ...
  \item a natural isomorphism with components $distEx_A:!eA\mto e!A$.
  \end{itemize}
\end{definition}



% Lemma: a linear category is embedded in a split linear category
\begin{lemma}
  \label{lem:linear-full-sub}
  Let $(\cat{L},!,e)$ be a split linear category. Define a full subcategory
  $\cat{L'}$ of $\cat{L}$ with object $eA$'s, $!eA$'s, and the unitor $I$
  in $\cat{L}$. Then $(\cat{L'},!,\w{},\c{})$ is a linear category.
\end{lemma}

\begin{proof}
  We prove the lemma by checking each component in the definition of a
  linear category (Definition~\ref{def:linear-category}).
  ...
  \item First, we need to show that $\cat{L'}$ is symmetric monoidal
    ...
    $\beta_{A,B}':!eA\otimes !eB\mto !eB\otimes !eA$ in $\cat{L'}$ as
    \begin{mathpar}
    \bfig
      \morphism<1000,0>[
        !eA\otimes !eB`
  ...
  % the monoidal comonad ! is symmetric on L'
  \item $(!,\varepsilon^!,\delta^!)$ is a symmetric monoidal comonad
    on $\cat{L'}$.
    
    Lemma~\ref{lem:compose-cw} has proved that $(!,\varepsilon^!,\delta^!)$
    is a monoidal comonad on $\cat{L}$. Since $\cat{L'}$ is monoidal, then
    $!$ is also monoidal on $\cat{L'}$. Now we only need to show it is
    symmetric, i.e. $(!,\q{}^!)$ is a symmetric monoidal endofunctor,
    and hence $\varepsilon^!$ and $\delta^!$ are symmetric monoidal natural
    transformations on $\cat{L'}$.
    
    $(!,\q{}^!)$ being symmetric on $\cat{L'}$ means the following
    diagram commutes:
    \begin{mathpar}
    \bfig
      \square<800,400>[
    ...
    \bfig
      \Vtriangle|amm|<500,500>[  
        ...
        e\varepsilon_A^!\otimes e\varepsilon_A^!]
      \Dtriangle(0,-500)|lmm|/->``<-/<500,500>[
        !eA\otimes !eB`
        eA\otimes eB`
        !(eA\otimes eB);
        \q{eA,eB}^!``
        \varepsilon_{eA\otimes eB}^!]
      \morphism(1000,500)<1100,0>[e!A\otimes e!B`e!B\otimes e!A;\e{!A,!B}]
    ...
    \efig
    \end{mathpar}
    , where (1) and (6) commute because $!$ is a monoidal comonad on
    $\cat{L'}$, (2) and (5) are definition of $dist_Ex$, (3) is the
    naturality of $\varepsilon^!$ and (4) is the naturality of $\e{}$.

    Then $\varepsilon^!$ and $\delta^!$ are also symmetric. 
  ...
  \item the three conditions of the monoidal natural transformations
        ...
        $\cat{L'}$ by replacing all object $cwA$ with $!eA$, $cwcwA$ with
        $!e!eA$, etc. and keeping the unitor $I$.
