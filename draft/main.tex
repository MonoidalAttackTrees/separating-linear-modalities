...
\begin{proof}
  The two diagrams above commute by the diagram chasings below:
  \begin{mathpar}
  \bfig
  ...
  \efig
  \end{mathpar}
  , which commute by the distribute law $dist$ and the comonad laws of $c$
  and $w$.
\end{proof}
...
\begin{definition}
  \label{def:Lambek-cw}
  A \textbf{Lambek category with $cw$}, $(\cat{L},cw,\w{}^w,\cL{},\cR{})$,
  is specified by
  \begin{itemize}
    \item a monoidal category $(\cat{L},I,\otimes)$;
  ...
  \begin{mathpar}
    ...
    \efig
  \end{mathpar}
  \todo{I modified this diagram by decomposing $\w{}$ as
        $\w{}^w\circ\varepsilon$ because we haven't defined $\w{}$ yet.}
\iffalse
  \begin{mathpar}
  \bfig
    \Vtriangle/->`->`->/<600,400>[
