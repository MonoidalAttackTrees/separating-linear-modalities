...
\begin{definition}
  ...
  \textbf{co-Kleisli Category of $e$}, $\cat{L}_e$, is a category with the
  same objects as $\cat{L}$. There is an arrow $f_e:A\mto B$ in $\cat{L}_e$
  if there is an arrow $f:eA\mto B$ in $\cat{L}$. The identity arrow
  $id_{A_e}:A\mto A$ is the arrow $\varepsilon_A:eA\mto A$ in $\cat{L}$.
  Given $f_e:A\mto B$ and $g_e:B\mto C$ in $\cat{L}_e$, which are arrows
  $f:eA\mto B$ and $g:eB\mto C$ in $\cat{L}$, the composition
  $g_e\circ f_e:A\mto C$ is defined as $g\circ ef\circ\delta_A$.
\end{definition}
